...
//            request.getHeaders().put(CatfishConstants.USER_ID, Collections.singletonList(umsUser.getId()));
            //将用户名放到request
//            request.getHeaders().put(CatfishConstants.USER_NAME, Collections.singletonList(umsUser.getUsername()));
            String ipAddr = "";
            ...
            ServerHttpRequest retRequest = exchange.getRequest()
                    ...
                    .header(CatfishConstants.USER_NAME, umsUser.getUsername())
                    .header(CatfishConstants.USER_IP, ipAddr)
                    //生成tokenB，非网关的微服务只需要解析tokenB，能解析出来则鉴权通过
                    .header(CatfishConstants.USER_TOKEN, jwtTokenManager.generateToken(umsUser.getUsername(), umsUser.getId()))
                    .build();
