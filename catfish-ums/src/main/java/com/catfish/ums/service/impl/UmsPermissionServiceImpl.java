...
public class UmsPermissionServiceImpl extends BaseServiceImpl<UmsPermissionMapper, UmsPermission> implements UmsPermissionService {
    ...
    @Autowired
    private UmsPermissionCategoryService umsPermissionCategoryService;
    ...
    @Override
    public List<UmsPermission> getByIds(Iterable<? extends Serializable> permissionIds){
    ...
    @Override
    public List<UmsPermission> getPermissionWithCategory(Iterable<String> ids) throws InvalidException {
        List<UmsPermission> umsPermissions = getByIds(ids);

        if(!CollectionUtils.isEmpty(umsPermissions)){
            Set<String> categoryIds = umsPermissions.stream().map(UmsPermission::getCategoryId).collect(Collectors.toSet());
            List<UmsPermissionCategory> umsPermissionCategory = umsPermissionCategoryService.getByIds(categoryIds);
            Map<String, String> umsPermissionCategoryMap = umsPermissionCategory.stream().collect(Collectors.toMap(UmsPermissionCategory::getId, UmsPermissionCategory::getName));
            umsPermissions.forEach(umsPermission -> {
                if(null != umsPermission.getCategoryId()){
                    umsPermission.setCategory(umsPermissionCategoryMap.get(umsPermission.getCategoryId()));
                }
            });
        }
    ...
    @Override
    public List<UmsPermission> getByRoleIds(Iterable<String> roleIds) {
