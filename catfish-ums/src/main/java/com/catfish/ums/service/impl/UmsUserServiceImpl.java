...
public class UmsUserServiceImpl extends BaseServiceImpl<UmsUserMapper, UmsUser> implements UmsUserService {
    ...
    public UserInfo getUserInfo() {
        ...
        List<String> rolesName = roles.stream().map(UmsRole::getName).collect(Collectors.toList());
    ...
    @Override
    public int addRoles(String userId, List<String> roleIds) throws InvalidException {
    ...
    @Override
    public int addResources(String userId, List<String> resourceIds) throws InvalidException {
        if (null == userId || !mapper.existsWithPrimaryKey(userId)) { //用户必须存在
            log.warn("user not exist, userId:{}", userId);
            throw new InvalidException(ReturnCodeEnum.USER_NOT_EXIST);
        }
    ...
    @Override
    public int addPermissions(String userId, List<String> permissionIds, Boolean status) throws InvalidException {
        if (null == userId || !mapper.existsWithPrimaryKey(userId)) { //用户必须存在
            log.warn("user not exist, userId:{}", userId);
            throw new InvalidException(ReturnCodeEnum.USER_NOT_EXIST);
        }
    ...
    @Override
    public List<UmsResource> getUserResources(String userId) {
    ...
    @Override
    public List<UmsPermission> getUserPermissions(String userId) {
    ...
    @Override
    public List<UmsRole> getRoles(String userId) {
    ...
    public String login(String userName, String password) {
        ...
        String token = jwtTokenUtils.generateToken(userDetails.getRetUser().getUsername(), userDetails.getRetUser().getId());
...
//        int length = token.length();//token长度是200，取前缀168个
    ...
    @Override
    public int save(UmsUser record) {
